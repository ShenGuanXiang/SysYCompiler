...
test:app
	...
	do
		...
		timeout 180s $(BINARY) $${file} -o $${ASM} -S 2>$${LOG} -O2; \
		RETURN_VALUE=$$?; \
		...
		if [ $$? != 0 ]; then
			...
			if [ -f "$${IN}" ]; then \
				timeout 120s qemu-arm -L /usr/arm-linux-gnueabihf $${BIN} <$${IN} >$${RES} 2>>$${LOG}; \
			else \
				timeout 120s qemu-arm -L /usr/arm-linux-gnueabihf $${BIN} >$${RES} 2>>$${LOG}; \
			fi; \
			RETURN_VALUE=$$?; \
			exec_end=$$(date +%s.%3N); \
