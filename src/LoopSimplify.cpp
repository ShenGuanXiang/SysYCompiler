...
void SimpleLoop::simplify()
{
    ...
    auto _mulmodtype = new FunctionType(TypeSystem::intType, std::vector<Type *>{TypeSystem::intType, TypeSystem::intType, TypeSystem::intType});
    // 1. 生成循环次数
    ...
    if (inductions[exit_var].modulo)
    {
        new FuncCallInstruction(tmp, std::vector<Operand *>{loop_time, inductions[exit_var].step, inductions[exit_var].modulo},
                                new IdentifierSymbolEntry(_mulmodtype, "_mulmod", 0), body);
    }
    else
