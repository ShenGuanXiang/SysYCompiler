...
std::vector<LoopInfo> findLoopInfo(Function *func)
{
    ...
    {
        LoopInfo loop;
        loop.loop_blocks = loop_helper->loop_bbs;
        loop.loop_header = loop_helper->header_bb;
        loop.loop_exiting_block = loop_helper->exiting_bb;

        // // 排除非最外层循环 TODO：从外到里找到第一层可并行的循环
        // if (loop_helper->parentLoop != nullptr)
        // {
        //     continue;
        // }

        // 排除while(a or b)
        if (la_helper.multiOr(loop_helper))
            continue;

        // 排除break/while(a and b)
        if (la_helper.hasLeak(loop_helper))
            continue;

        // 判断归纳变量是否唯一、变化形式并记录
        bool flag = true;
        for (auto ind : loop_helper->inductionVars)
        {
            ...
            for (auto inst : du_chain)
            {
                ...
                {
                    if (!loop_helper->loop_bbs.count(use->getParent()))
                    {
                        flag = false;
                        break;
                    }
        ...
        if (loop_helper->inductionVars.size() != la_helper.getLoopCnt(loop_helper))
            continue;

        assert(loop.loop_exiting_block->rbegin()->isCond());
        assert(loop.loop_exiting_block->rbegin()->getUses()[0]->getDef() && loop.loop_exiting_block->rbegin()->getUses()[0]->getDef()->isCmp());
        loop.cmp = loop.loop_exiting_block->rbegin()->getUses()[0]->getDef();

        InductionVar *ind_var = nullptr;
        for (auto ind : loop_helper->inductionVars)
        {
            auto du_chain = ind->du_chains[0];
            if (loop.cmp->getUses()[0] == du_chain[1]->getDef() || loop.cmp->getUses()[1] == du_chain[1]->getDef())
            {
                ind_var = ind;
                break;
            }
        ...
        if (!range_second->getType()->isConst() && range_second->getDef() && loop_helper->loop_bbs.count(range_second->getDef()->getParent()))
            continue;
        assert(du_chain[0]->isPHI());
        assert(du_chain[1]->isBinary());
        auto stride = du_chain[1]->getUses()[0] == du_chain[0]->getDef() ? du_chain[1]->getUses()[1] : du_chain[1]->getUses()[0];
        if (!stride->getType()->isConst() && stride->getDef() && loop_helper->loop_bbs.count(stride->getDef()->getParent()))
            continue;
        loop.phi = du_chain[0];
        auto phi_srcs = dynamic_cast<PhiInstruction *>(loop.phi)->getSrcs();
        assert(phi_srcs.size() == 2);
        assert(phi_srcs.count(loop.loop_exiting_block));
        auto range_first = loop.phi->getUses()[0] == phi_srcs[loop.loop_exiting_block] ? loop.phi->getUses()[1] : loop.phi->getUses()[0];
