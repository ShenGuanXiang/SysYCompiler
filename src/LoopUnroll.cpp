...
//     PhiInstruction* InductionVarphi = nullptr;
//     assert(cmp->isCmp());
//     std::vector<Instruction *> old_instrs, old_phis;
//     for (auto instr = bb->begin(); instr != bb->end(); instr = instr->getNext())
//     {
//         if (instr->isPHI()) {
//             old_phis.push_back(instr);
//             if (((PhiInstruction*)instr)->getSrcs().count(bb))
//                 InductionVarphi = (PhiInstruction*)instr;
//         }
//         if (instr->isCond() || instr->isUncond()) 
//             continue;
//         old_instrs.push_back(instr);
//     }

//     std::map<Operand*, Operand*> ope2ope;
//     for (auto old_instr : old_instrs) {
//         Instruction* ins = old_instr->copy();
//         ins->setParent(old_instr->getParent());
//         if (!old_instr->hasNoDef()) {
//             Operand* newDef = copyOperand(old_instr->getDef());
//             newDef->setDef(old_instr->getDef()->getDef());
//             ope2ope[old_instr->getDef()] = newDef;
//             ins->setDef(newDef);
//         }
