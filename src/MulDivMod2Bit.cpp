               ...
               if ( (curr_inst->isMov() ||  (curr_inst->isLoad() && ((LoadMInstruction*)curr_inst)->is_1_src())) && next_inst->isMul() ) 
                {
                    ...
                    if ( (*mulSrc1 == *Dst || *mulSrc2 == *Dst)  
                        && Src->isImm() && !Src->getValType()->isFloat()
                        && Src->getVal()!=0
                        && is2Exp(Src->getVal())) 
                    {
                        ...
                        auto mov_lsl = new MovMInstruction( block, MovMInstruction::MOVLSL, mulDst, lslSrc); //mov r6, r4, LSL#1
                        mov_lsl->mov_num = log2(Src->getVal());
                ...
                if (next_inst_iter + 1 != block->end()) 
                {
                    ...
                    if ( (curr_inst->isMov() ||  (curr_inst->isLoad() && ((LoadMInstruction*)curr_inst)->is_1_src())) && next_next_inst->isMul() ) 
                    {
                            ...
                            && is2Exp(Src->getVal())) 
                        {
                            ...
                            auto mov_lsl = new MovMInstruction( block, MovMInstruction::MOVLSL, mulDst, lslSrc); //mov r9, r8, LSL#1
                            mov_lsl->mov_num = log2(Src->getVal());
