...
Operand *GVNPRE::find_leader(Exprset set, ValueNr val)
{
    ...
    if(val->getEntry()->isConstant()) return val; //all literal constant has its leader anywhere
    if(val->getEntry()->isVariable()) return val; //function param, global also hold
    if(!set.count(val)) return nullptr;
    else return set[val].vals[0];
    //assert(set[val].op==0)
    ...
    while (changed){
        ...
        while(!q.empty()){
            ...
            for(auto it=tmp_gen[bb].begin();it!=tmp_gen[bb].end();it++){
                auto val = it->first;
                auto e = it->second;
                if(S.count(val) && S[val]==e) S.erase(val);
            }
            antic_in[bb]=expr_gen[bb];
            for(auto it=tmp_gen[bb].begin();it!=tmp_gen[bb].end();it++){
                auto val = it->first;
                auto e = it->second;
                if(antic_in[bb].count(val) && antic_in[bb][val]==e) antic_in[bb].erase(val);
            }
            ...
            if(old!=antic_in[bb]) changed=true;
